class Ability
  ...
  def can_administer_any_claim_in_provider(persona)
    ...
    can %i[show show_message_controls messages edit update summary unarchive confirmation clone_rejected destroy disc_evidence], Claim::BaseClaim, provider_id: persona.provider_id
  ...
  def can_manage_own_claims_of_class(persona, claim_klass)
    ...
    can %i[show show_message_controls messages edit update summary unarchive confirmation clone_rejected destroy disc_evidence], claim_klass, external_user_id: persona.id
    can %i[show create update], Certification
