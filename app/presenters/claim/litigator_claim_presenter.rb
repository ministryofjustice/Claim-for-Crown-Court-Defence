class Claim::LitigatorClaimPresenter < Claim::BaseClaimPresenter
  ...
  def raw_grad_fees_vat
    VatRate.vat_amount(raw_grad_fees_total, claim.created_at, calculate: claim.apply_vat?)
  end

  def raw_grad_fees_gross
    raw_grad_fees_total + raw_grad_fees_vat
  end

  def grad_fees_vat
    h.number_to_currency(raw_grad_fees_vat)
  end

  def grad_fees_gross
    h.number_to_currency(raw_grad_fees_gross)
  end
