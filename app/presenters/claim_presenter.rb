class ClaimPresenter < BasePresenter
  ...
  def history_and_messages
    dates = (messages.map(&:created_at) + versions.map(&:created_at)).compact.uniq.sort
    date_strings = dates.map { |d| d.strftime(Settings.date_format) }.uniq

    date_strings.inject({}) do |hash, date_string|
      arr = select_by_date_string(versions, date_string) + select_by_date_string(messages, date_string)
      arr = arr.flatten.sort_by { |e| e.created_at }
      hash[date_string] = arr if arr.any?
      hash
    end
  end
