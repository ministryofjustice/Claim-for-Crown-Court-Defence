  ...
  module Establishments
    class PrisonSeeder
      ...
      def process_row(row)
        ...
        establishment = Establishment.find_by(category: 'prison', name: name)
      ...
      def create_record(name, attributes)
        Establishment.create(attributes.merge(name: name, category: 'prison')) unless dry_run
        log "[CREATED] Created prison with name '#{name}'"
        self.total_created += 1
      end

      def update_record(record, name, attributes)
        record.update(attributes) unless dry_run
        log "[UPDATED] Updated prison with name '#{name}'"
