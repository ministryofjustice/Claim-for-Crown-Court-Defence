  ...
  module Schemas
    class AddAGFSFeeScheme17
      ...
      def create_agfs_scheme_seventeen
        print Rainbow("Finding AGFS scheme 16").yellow
      ...
      def destroy_agfs_scheme_seventeen
        if pretending?
          puts Rainbow("Would delete fee scheme 17: #{agfs_fee_scheme_17&.attributes || 'does not exist'}").yellow
          puts Rainbow("Would update #{agfs_fee_scheme_16.attributes} end date to nil").yellow
        else
          puts Rainbow('Deleted fee scheme 17').red if agfs_fee_scheme_17&.destroy
          puts Rainbow('Updated fee scheme 16 end date to nil').green if agfs_fee_scheme_16&.update(end_date: nil)
        end
      end
      ...
      def create_scheme_seventeen_fee_types
        puts Rainbow("Scheme 17 fee type count before: #{scheme_17_fee_type_count}").yellow
        require Rails.root.join('db', 'seeds', 'fee_types', 'csv_seeder')
        Seeds::FeeTypes::CsvSeeder.new(dry_mode: pretending?, stdout: false).call
        puts Rainbow("Scheme 17 fee type count after: #{scheme_17_fee_type_count}").yellow
      end
      ...
      def remove_scheme_seventeen_fee_type_roles
        ...
        if pretending?
          puts Rainbow("Would remove agfs_scheme_17 role from #{fee_types_with_scheme_17_role.count} fee_types").yellow
        else
          ActiveRecord::Base.transaction do
            fee_types_with_scheme_17_role.each do |ft|
              if ft.roles == ['agfs_scheme_17']
                ...
                puts Rainbow("Removing agfs_scheme_17 role from #{ft.description}").green
                ft.roles.delete('agfs_scheme_17')
                ft.save!
              end
            end
          end
          puts Rainbow("Fee types with scheme 17 role after: #{fee_types_with_scheme_17_role.count}").green
        end
      end
    end
  end
