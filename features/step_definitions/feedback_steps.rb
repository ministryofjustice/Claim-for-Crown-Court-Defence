...
Then(/^I see confirmation that my '(.*?)' was received$/) do |feedback_type|
  ...
  when 'bug report'
    expect(page).to have_govuk_notification_banner(key: :notice, text: 'Bug Report submitted')
  end
end
...
Then('I see a warning that my bug report was not submitted successfully') do
  expect(page).to have_govuk_notification_banner(key: :error, text: /Unable to submit bug report/)
end
