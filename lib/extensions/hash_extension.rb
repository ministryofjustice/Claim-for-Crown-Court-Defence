module HashExtension
  def all_keys
    each_with_object([]) do |(k, v), keys|
      keys << k
      keys.concat(v.select{ |el| el.is_a?(Hash) }.map(&:all_keys).flatten(1)) if v.is_a? Array
  ...
  def all_values_for(key)
    each_with_object([]) do |(k, v), values|
      ...
      values.concat(v.select{ |el| el.is_a?(Hash) }.map{ |el| el.all_values_for(key) }.flatten(1)) if v.is_a? Array
