...
namespace :db do
  namespace :agfs_scheme_seventeen do
    desc 'Display the status of db structures and records for AGFS Fee Scheme 17'
    task status: :environment do
      adder = Seeds::Schemas::AddAGFSFeeScheme17.new(pretend: false)
      puts adder.status
    end

    desc 'Create the db structures and records required for AGFS Fee Scheme 17, pass \'true\' to run'
    ...
    desc 'Destroy the db structures and records required for AGFS Fee Scheme 17, pass \'true\' to run'
    task :rollback, [:not_pretend] => :environment do |_task, args|
      ...
      continue?('This will destroy AGFS Fee Scheme 17, offences and fee types. Are you sure?') if not_pretend
      puts "#{pretend ? 'pretending' : 'working'}...".yellow
