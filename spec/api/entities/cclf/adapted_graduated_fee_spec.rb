...
RSpec.describe API::Entities::CCLF::AdaptedGraduatedFee, type: :adapter do
  ...
  let(:fee_type) { instance_double(Fee::BaseFeeType, unique_code: 'GRTRL') }
  let(:case_type) { instance_double(CaseType, fee_type_code: 'GRTRL') }
  let(:claim) { instance_double(Claim::BaseClaim, case_type:) }
  let(:graduated_fee) { instance_double(Fee::GraduatedFee, claim:, fee_type:, quantity: 999.0) }
  ...
  context 'when zero quantity on grad fee' do
    let(:graduated_fee) { instance_double(Fee::GraduatedFee, claim:, fee_type:, quantity: 0) }
