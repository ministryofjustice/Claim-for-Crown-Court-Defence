...
RSpec.describe API::V2::MI::AGFSSchemeTenClaims do
  ...
  subject(:do_request) { get '/api/mi/scheme_10_claims', params.merge!(transitory_params), format: :json }
  ...
  before do
    ...
    travel_to Date.parse('21-APR-2018')
  end
  ...
  context 'when accessed by a CaseWorker' do
    let(:api_key) { create(:case_worker, :admin).user.api_key }
  ...
  context 'when accessed by a ExternalUser' do
    ...
    let(:api_key) { create(:external_user).user.api_key }
