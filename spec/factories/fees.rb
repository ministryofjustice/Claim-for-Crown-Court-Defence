...
FactoryBot.define do
  factory :fixed_fee, class: 'Fee::FixedFee' do
    claim { association :advocate_claim, :with_fixed_fee_case }
    fee_type { association :fixed_fee_type }
    quantity { 1 }
    rate { 25 }

    trait :lgfs do
      claim { association :litigator_claim, :with_fixed_fee_case }
      fee_type { association :fixed_fee_type, :lgfs }
      quantity { 1 }
      rate { 25 }
      date { 3.days.ago }
    end
  ...
  factory :misc_fee, class: 'Fee::MiscFee' do
    claim
    fee_type { association :misc_fee_type }
    quantity { 1 }
    rate { 25 }

    trait :lgfs do
      fee_type { association :misc_fee_type, :lgfs }
      quantity { 0 }
      rate { 0 }
      amount { 25 }
    end
    ...
    trait :spf_fee do
      fee_type { association :misc_fee_type, :spf }
    end
  ...
  factory :warrant_fee, class: 'Fee::WarrantFee' do
    claim
    fee_type { association :warrant_fee_type }
  ...
  factory :interim_fee, class: 'Fee::InterimFee' do
    claim { association :interim_claim }
    fee_type { association :interim_fee_type }
    quantity { 2 }
    amount { 245.56 }
    ...
    trait :disbursement do
      claim { association :interim_claim, disbursements: build_list(:disbursement, 1) }
      fee_type { association :interim_fee_type, :disbursement_only }
      amount { nil }
      quantity { nil }
    end

    trait :warrant do
      fee_type { association :interim_fee_type, :warrant }
      quantity { nil }
      amount { 25.02 }
    ...
    trait :effective_pcmh do
      fee_type { association :interim_fee_type, :effective_pcmh }
      quantity { nil }
    end

    trait :trial_start do
      fee_type { association :interim_fee_type, :trial_start }
      quantity { 1 }
    end

    trait :retrial_start do
      fee_type { association :interim_fee_type, :retrial_start }
      quantity { 1 }
    end

    trait :retrial_new_solicitor do
      fee_type { association :interim_fee_type, :retrial_new_solicitor }
      quantity { nil }
    end
  end
  ...
  factory :basic_fee, class: 'Fee::BasicFee' do
    claim
    fee_type { association :basic_fee_type }
    quantity { 1 }
    rate { 25 }
  ...
  factory :transfer_fee, class: 'Fee::TransferFee' do
    claim { association :transfer_claim }
    fee_type { association :transfer_fee_type }
    quantity { 0 }
    rate { 0 }
    amount { 25 }
  end
  ...
  factory :graduated_fee, class: 'Fee::GraduatedFee' do
    claim
    fee_type { association :graduated_fee_type }
    quantity { 1 }
    amount { 25 }
    rate { 0 }
    date { 3.days.ago }
    ...
    trait :guilty_plea_fee do
      fee_type { association :graduated_fee_type, description: 'Guilty plea', code: 'GGLTY' }
    end

    trait :trial_fee do
      fee_type { association :graduated_fee_type, :grtrl }
    end
  end

  factory :hardship_fee, class: 'Fee::HardshipFee' do
    claim
    fee_type { association :hardship_fee_type }
    quantity { 1 }
    amount { 25 }
  end
