  ...
  describe '.code' do
    ...
    context 'when the offence is in' do
      ...
      context 'scheme ten' do
        let(:offence) { create(:offence, :with_fee_scheme_ten, description: 'Murder', offence_band:) }

        it { is_expected.to eql 'MURDER_1.1' }
      end

      context 'scheme eleven' do
        let(:offence) { create(:offence, :with_fee_scheme_eleven, description: 'Murder', offence_band:) }
