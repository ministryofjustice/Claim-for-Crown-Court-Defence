    ...
    describe 'valid_transfer_stage_ids' do
      context 'new litigator type with elected case' do
        it 'returns a list of valid transfer_stage_ids' do
          detail = build :transfer_detail, litigator_type: 'new', elected_case: true
          expect(TransferBrainDataItemCollection.instance.valid_transfer_stage_ids(detail)).to eq([ 10, 20, 50 ])
        end
      end

      context 'new litigator type and non elected case' do
        it 'returns a list of valid transfer_stage_ids' do
          detail = build :transfer_detail, litigator_type: 'new', elected_case: false
          expect(TransferBrainDataItemCollection.instance.valid_transfer_stage_ids(detail)).to eq([10, 20, 30, 40, 50, 60, 70])
        end
      end
    end
