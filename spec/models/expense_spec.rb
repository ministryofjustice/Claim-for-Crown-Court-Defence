...
RSpec.describe Expense do
  ...
  context 'expense_reasons and expense reason text' do
    let(:basic_expense) { build(:expense, reason_id: 1) }
    let(:expense_with_nil_id) { build(:expense, reason_id: nil) }
    let(:expense_with_reason) { build(:expense, reason_id: 5, reason_text: 'My unique reason') }

    describe '#expense reason' do
      it 'returns the reason object with id 1' do
        expect(basic_expense.expense_reason).to be_instance_of(ExpenseReason)
        expect(basic_expense.expense_reason.id).to eq 1
      end

      it 'returns nil if reason_id not set' do
        expect(expense_with_nil_id.expense_reason).to be_nil
      end
    end

    describe '#allow_reason_text' do
      it 'returns false if no reason id' do
        expect(expense_with_nil_id.allow_reason_text?).to be false
      end

      it 'returns false for reason id 1' do
        expect(basic_expense.allow_reason_text?).to be false
      end

      it 'returns true for reason id 5' do
        expect(expense_with_reason.allow_reason_text?).to be true
      end
    end

    describe '#displayable_reason_text' do
      it 'returns nil if reason id is nil' do
        expect(expense_with_nil_id.displayable_reason_text).to be_nil
      end

      it 'returns reason from reason text' do
        expect(basic_expense.displayable_reason_text).to eq 'Court hearing'
      end

      it 'returns the reason_text from the record for reason id 5' do
        expect(expense_with_reason.displayable_reason_text).to eq 'My unique reason'
      end
    end
  end
