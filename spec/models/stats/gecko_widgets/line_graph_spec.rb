  ...
  module GeckoWidgets
    describe 'LineGraph' do
      ...
      describe '#dataaset_names' do
        it 'returns an array of dataset names in the order they were added' do
          expect(@lg.dataset_names).to eq %w{Tories Labour LibDems}
        end
      end

      describe '#dataset size' do
        it 'returns the number of entries in the data series' do
      ...
      describe '#to_json' do
        context 'without specifying x-axis labels' do
          it 'produces a json structure suitable for generating a Geckoboard Line graph widget' do
            data = {
      ...
      describe '#to_a' do
        context 'without specifying x-axis labels' do
          it 'produces an array of arrays suitable for displaying in a tabular format' do
