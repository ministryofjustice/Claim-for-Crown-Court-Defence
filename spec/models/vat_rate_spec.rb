...
RSpec.describe VatRate do
  ...
  describe '.for_date' do
    it 'returns 8% for dates between 1 and 3 years ago' do
      expect(VatRate.for_date(2.years.ago)).to eq 800
    end

    it 'returns 17.5% for dates between 3 and 10 years ago' do
      expect(VatRate.for_date(4.years.ago)).to eq 1750
    end

    it 'returns 22.25% for dates less than one year ago' do
      expect(VatRate.for_date(3.months.ago)).to eq 2225
    end

    it 'raises exception for date before the first date in the database' do
      expect {
