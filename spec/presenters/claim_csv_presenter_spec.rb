...
RSpec.describe ClaimCsvPresenter do
  ...
  let(:advocate)            { double 'advocate', supplier_number: '0P235X', provider: provider}
  ...
  let(:claim)               { double 'claim', case_number: 'A12345678', advocate: advocate, case_type: case_type, total: BigDecimal.new(100), state: 'submitted', last_submitted_at: last_submitted_at, versions: versions, redetermination?: false, awaiting_written_reasons?: false, opened_for_redetermination?: true}
  ...
  context '#present! generates csv that contains' do
    ...
    it 'account number' do
      expect(subject.present!).to include(claim.advocate.supplier_number)
    end

    it 'organistion/provider_name' do
      expect(subject.present!).to include(claim.advocate.provider.name)
    end
    ...
    it 'case_type' do
      expect(subject.present!).to include(claim.case_type.name)
    end
