...
RSpec.describe ErrorMessage::Presenter do
  ...
  describe '#field_errors_for' do
    ...
    context 'with a nested error that is generated by BaseSubModelValidator' do
      before do
        pending 'implementation of this functionality as part of a govuk form migration'
        claim.errors.add(attribute, 'foo_bar')
      end

      let(:attribute) { :'graduated_fee.date' }
  ...
  describe '#formatted_error_messages' do
    ...
    context 'with a nested error that is generated by BaseSubModelValidator' do
      before do
        pending 'implementation of this functionality as part of a govuk form migration'
        claim.errors.add(:'graduated_fee.date', 'check_not_in_future')
        claim.errors.add(:graduated_fee_attributes_date, 'Graduated fee date cannot be too far in the future')
      end

      let(:expected_error_messages) do
        [
          [:graduated_fee_attributes_date, 'Graduated fee date cannot be too far in the future']
        ]
      end
