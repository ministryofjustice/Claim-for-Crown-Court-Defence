...
RSpec.shared_examples_for 'a hardship fee adapter' do
  describe '#mappings' do
    subject { instance.mappings }

    %i[BABAF BADAF BADAH BADAJ BANOC BANDR BANPW BAPPE].each do |basic_fee_unique_code|
      it "includes mappings for basic fee #{basic_fee_unique_code} to a CCR Advocate Fee bill type - AGFS_HARDSHIP" do
        is_expected.to include(basic_fee_unique_code => { bill_type: 'AGFS_ADVANCE', bill_subtype: 'AGFS_HARDSHIP'})
      end
    end
  end
  ...
  describe '#bill_type' do
    ...
    it 'returns CCR Advocate Fee bill type' do
      is_expected.to eql 'AGFS_ADVANCE'
    end
  end
  ...
  describe '#bill_subtype' do
    ...
    it 'returns CCR Advocate Fee bill subtype' do
      is_expected.to eql 'AGFS_HARDSHIP'
    end
  end
end
...
RSpec.describe CCR::Fee::HardshipFeeAdapter, type: :adapter do
  context 'when instantiated without a claim object' do
    ...
    it_behaves_like 'a hardship fee adapter' do
