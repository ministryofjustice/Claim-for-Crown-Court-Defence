...
describe ClaimDateValidator do 
  ...
  context 'trial_fixed_at' do
    it 'should error if not present for cracked trial' do
      expect(cracked_trial_claim.valid?).to be false
      expect(cracked_trial_claim.errors[:trial_fixed_at]).to eq( [ 'Please enter valid date of first fixed/warned' ])
    end

    it 'should error if not present for cracked before retrial' do
      expect(cracked_before_retrial_claim.valid?).to be false
      expect(cracked_before_retrial_claim.errors[:trial_fixed_at]).to eq( [ 'Please enter valid date of first fixed/warned' ])
    end

    it 'should error if in the future' do
      cracked_trial_claim.trial_fixed_at = 3.days.from_now.to_date
      expect(cracked_trial_claim.valid?).to be false
      expect(cracked_trial_claim.errors[:trial_fixed_at]).to eq( [ 'Date of first fixed/warned may not be in the future' ])
    end

    it 'should error if more than 5 years old' do
      cracked_trial_claim.trial_fixed_at = 61.months.ago.to_date
      expect(cracked_trial_claim.valid?).to be false
      expect(cracked_trial_claim.errors[:trial_fixed_at]).to eq( [ 'Date of first fixed/warned may not be older than 5 years' ])
    end

    it 'should error if before earliest rep order' do
      repo = double RepresentationOrder
      allow(cracked_trial_claim).to receive(:earliest_representation_order).and_return(repo)
      allow(repo).to receive(:representation_order_date).and_return(1.year.ago.to_date)
      cracked_trial_claim.trial_fixed_at = 13.months.ago.to_date
      expect(cracked_trial_claim.valid?).to be false
      expect(cracked_trial_claim.errors[:trial_fixed_at]).to eq( [ 'Date of first fixed/warned may not be earlier than the first representation order date' ])
    end

    it 'should error if before the trial_fixed_notice_at' do
      cracked_trial_claim.trial_fixed_notice_at = 180.days.ago.to_date
      cracked_trial_claim.trial_fixed_at = 181.days.ago.to_date
      expect(cracked_trial_claim.valid?).to be false
      expect(cracked_trial_claim.errors[:trial_fixed_at]).to eq( [ 'Date of first fixed/warned may not be earlier than the date notice of first fixed/warned issued' ])
    end
